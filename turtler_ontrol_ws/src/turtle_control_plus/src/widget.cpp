...
Widget::Widget(ros::NodeHandle *node,QWidget *parent) :
...
void Widget::initWidgets() {
    ...
    connect(updateTimer, &QTimer::timeout, this, &Widget::on_timer_update_data);
    ...
    //添加事件
    btnSend->connect(btnSend, &QPushButton::clicked, this, &Widget::on_btn_click_Send);
    ...
    subscriber = node->subscribe(poseTopicName, 1000, &Widget::poseCallback, this);
}

void Widget::on_timer_update_data()
{
    ros::spinOnce();
...
void Widget::poseCallback(const turtlesim::Pose_<std::allocator<void>>::ConstPtr &message) {
    double x = message->x;
    double y = message->y;
